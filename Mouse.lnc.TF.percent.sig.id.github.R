...
setwd("E:/Postdoc/Project/MmAdilnc/Regulatory/TFBS/lncPmo.JASPAR.vet.nr.fimo.out")


#input lncRNA id
allconlncname=read.table(file = "E:/Postdoc/pub/HMconserlnc/Mm.conlnc.EnsembleID.txt",
                         header = F,stringsAsFactors = F)$V1

nonconlncname=read.table(file = "E:/Postdoc/Project/MmAdilnc/geneid/Mmnonconlnc.EnsembleID.txt",
                         header = F,stringsAsFactors = F)$V1
...
gtype=read.table(file = "E:/Postdoc/pub/GeneCode/gencode.vM17.pro.lncAddADI.annotation.gtype.txt",
...
all.lnc.JASPAR.percent$convsnoncon.log2FC=log2((all.lnc.JASPAR.percent$lnc_con_all+all.lnc.per.plus.0.5thd)/(all.lnc.JASPAR.percent$lnc_noncon+all.lnc.per.plus.0.5thd))
...
all.lnc.JASPAR.top10per.sig=subset(all.lnc.JASPAR.percent,
                                   ((abs(convsnoncon.minus) > all.lnc.minus.thd) | (abs(convsnoncon.log2FC) > all.lnc.log2FC.thd & convsnoncon.mean > all.lnc.mean.thd)) & allvsnoncon.Pval < 0.05)


#Result output
write.table(all.lnc.JASPAR.top10per.sig,file = "./all.lnc.JASPAR.percent.top10per.sig.xls",sep = "\t",quote = F,col.names = NA)
...
write.table(all.lnc.JASPAR.top10per.sig.lnc.TF,file = "./all.lnc.JASPAR.percent.top10per.sig.lnc.TF.xls",
